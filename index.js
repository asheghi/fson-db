...
module.exports = function (storagePath) {
			...
			try {
				...
				if (typeof value === "object") {
					...
					let nestedHandler = (arg, parentObject, np) => ({
						...
						set: (t, n, v) => {
							let obj = this.localStorage.getItem(name);
							let path = [...arg, n];
							if (path.length) {
								const sections = path;
								let temp = obj;
							...
							this.localStorage.setItem(name, obj);
						},
					});
					let target = value;
	...
	mainHandler.localStorage = createLocalStorage(path.resolve(storagePath));
